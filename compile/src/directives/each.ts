...
import {
  parseInputOutput, parseOutput, parseInputObjectExpression, srzOutput, srzInputOutput,
...
export default function processEach(ctx: IContext, content: string): void {
  try {
    const { input, output, mock } = parseInputOutput(content);
...
function parseInputExpr(content: string): { array?: Identifier | MemberExpression } {
  ...
  let expr: Expression;
  if (content[0] === '{') {
    const { array } = parseInputObjectExpression(content);
    if (!array) { throw new SyntaxError('each指令输入参数需要array'); }
    expr = array;
  } else {
    expr = parseExpressionAt(content) as Expression;
  }
...
export function srzEachValue({ input, output, mock }: IDirectiveEach): string {
  const inputStr = input.expr ? srzInputExpr(input.expr) : input.content;
  const outputStr = output.expr ? srzOutput(output.expr) : output.content;
  const mockStr = mock.expr ? srzMockExpr(mock.expr) : mock.content;
  return srzInputOutput(inputStr, outputStr, mockStr);
}

function srzInputExpr(expr: NonNullable<IDirectiveEach['input']['expr']>) {
  if (!expr.array) {
    return '';
  }
  return generate(expr.array);
}
